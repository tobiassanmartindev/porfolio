...
function handleMouseMove(e) {
  if (!props.eyeOpen) return
  if (!eyeRef.value || !pupilRef.value) return
  ...
  const radius = Math.min(3, Math.hypot(dx, dy))

  const pupilX = Math.cos(angle) * radius
  const pupilY = Math.sin(angle) * radius

  pupilRef.value.style.transform = `translate(${pupilX}px, ${pupilY}px)`
}
    ...
    <div
      ...
      >
        ...
        <div
          ref="eyeRef"
          class="eye"
          :class="{ 'eye-open': props.eyeOpen, 'eye-closed': !props.eyeOpen }"
        >
          <div ref="pupilRef" class="pupil"></div>
        </div>

        <div
          style="margin-left: -50px"
          class="absolute inset-0 z-20 w-[400px] h-[325px] 
            bg-[linear-gradient(to_top,#0f172a_30%,transparent_100%)]"
        ></div>
...
.eye {
  ...
  justify-content: center;
  align-items: center;
  transform-origin: top;   /* 🔑 El ojo se abre/cierra desde arriba */
  transition: transform 0.02s ease;
  margin-left: -6px;
}

.eye-open {
  transform: scaleY(1); /* ojo completamente abierto */
}

.eye-closed {
  transform: scaleY(0); /* ojo cerrado (aplastado desde arriba) */
}
